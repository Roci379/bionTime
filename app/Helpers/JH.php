...
use Javfres\Slack\Facades\Slack;

class JH {

    protected static $obj_dict = [];

    public static function log($something, $full=false){
        ...
        self::$obj_dict = [];
        error_log(self::better_print_r($something,0,$full));

    }


    public static function better_print_r($var, $level=0, $full=false){

        if(is_object($var)){
            $hash = spl_object_hash($var);
            if(isset(self::$obj_dict[$hash])){
                return "!!!!! LOOP !!!!!";
            }

            self::$obj_dict[$hash] = true;
        }
        ...
        if (is_array($var)) {
            $out = "Array (\n";
            $out .= self::better_print_r_array_items($var, $full, $level);
            $out .= $spaces . ")";
            return $out;
        }
        ...
        if (is_object($var) && $var instanceof Model) {
            ...
            $out .= self::better_print_r_array_items($var->toArray(), $full, $level);
            $out .= $spaces . ")";
            return $out;
        }
    ...
    private static function better_print_r_array_items($array, $full, $level = 0)
    {

        $spaces = str_repeat(" ", 4 * $level);
        $out = "";
        foreach ($array as $k => $v) {
            $out .= "$spaces    [$k] => " . self::better_print_r($v, $level + 1, $full) . "\n";
        }
    ...
    public static function deprecated($msg = 'This is deprecated', $throw=true){
        if(App::environment('portal')){
            ...
            try {
                ...
                Slack::from("Zombie")->to('#wseq_test')
                    ->withIcon(':zombie:')
                    ->send("Using deprecated code: `" . $msg . "`" . ($user ? " by $user" : ""));
            } catch (Exception $e) {
            }
            return;
        }
    ...
    public static function log_memory($msg = ''){
        ...
        self::log($msg. "Memory usage: $mb Mb, Max memory used: $mb_max Mb");
